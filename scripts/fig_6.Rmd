...
#### Selecting MER11 loci and genes for CRISPRi targeting

```{r CRISPRi_target_selection}
...
starr_pks_abc_te_ovls %>% 
  ...
  ungroup() #%>% 
  # slice_head(n = 50) %>% 
  # write_tsv("230227_GP5d_MER11ABC_CRISPRi_candidate_list.tsv")
...
pathway <- fgsea::gmtPathways("/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/gsea_pathways/h.all.v2023.1.Hs.symbols.gmt")

mer11_targets <- starr_pks_abc_te_ovls %>% 
  filter(str_detect(te_name, "MER11[ABCD]")) %>% 
  dplyr::select(hgnc, l2fc_hcoepic) %>% 
  arrange(l2fc_hcoepic) %>% 
  na.omit() %>% 
  pull(l2fc_hcoepic, name = hgnc)

mer11_targets_gsea <- GSEA(mer11_targets, "/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/gsea_pathways/h.all.v2023.1.Hs.symbols.gmt", 0.05)

```


```{r}
cl_1_targets <- starr_pks_abc_te_ovls %>% 
  filter(cluster == 1) %>% 
  dplyr::select(hgnc, l2fc_hcoepic) %>% 
  distinct() %>% 
  arrange(l2fc_hcoepic) %>% 
...
cl_1_targets_gsea <- GSEA(cl_1_targets, "/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/gsea_pathways/h.all.v2023.1.Hs.symbols.gmt", 0.05)
```


```{r}
cl_1_te_targets <- starr_pks_abc_te_ovls %>% 
  filter(cluster == 1, !is.na(te_name)) %>% 
  dplyr::select(hgnc, l2fc_hcoepic) %>% 
  distinct() %>% 
  na.omit() %>% 
  filter(hgnc != "Y_RNA") %>% 
...
cl_1_te_targets_gsea <- GSEA(cl_1_te_targets, "/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/gsea_pathways/h.all.v2023.1.Hs.symbols.gmt", 0.05)
```

```{r}

# mer11a_starr_ovl <- repeatmasker[subjectHits(starr_te_ovl), ] %>% 
#   filter(te_name == "MER11A") %>% 
#   arrange(seqnames, start) %>% 
#   write_tsv("/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/te_subfs/GP5d/MER11_background_test/GP5d_mer11_starr_ovls.bed", col_names = F)
# 
# repeatmasker %>% 
#   filter(te_name == "MER11A") %>% 
#   filter(!name %in% mer11a_starr_ovl$name) %>% 
#   arrange(seqnames, start) %>% 
#   write_tsv("/data/kzkarttu/gradu/scripts/R_scripts/gradu/data/te_subfs/GP5d/MER11_background_test/GP5d_mer11_background.bed", col_names = F)
